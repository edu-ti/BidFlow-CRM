    ...
    <div className="flex h-[calc(100vh-6rem)] bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {mockConversations.map(conv => (
            <div 
              ...
              className={`p-4 flex items-center gap-3 cursor-pointer hover:bg-gray-50 transition ${selectedChat === conv.id ? 'bg-indigo-50' : ''}`}
            >
      ...
      <div className="hidden md:flex flex-1 flex-col bg-[#e5ddd5] relative">
        {selectedChat ? (
          <>
            {/* Header */}
            <div className="bg-gray-100 p-3 flex justify-between items-center border-b border-gray-200 z-10">
              <div className="flex items-center gap-3">
                <img src={mockConversations.find(c => c.id === selectedChat)?.avatar} className="w-10 h-10 rounded-full" alt="" />
                <div>
                  <h3 className="font-semibold text-gray-800">{mockConversations.find(c => c.id === selectedChat)?.contactName}</h3>
                ...
                <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] p-3 rounded-lg shadow-sm relative ${
                    ...
                    <div className="flex justify-end items-center gap-1 mt-1">
                      ...
                      {msg.sender === 'me' && (
                        msg.status === 'read' ? <CheckCheck size={14} className="text-blue-500" /> : <Check size={14} className="text-gray-500" />
                      )}
                    </div>
                  </div>
                </div>
            ...
            {/* Input Area */}
            <div className="bg-gray-100 p-3 flex items-center gap-3 z-10">
              ...
              <input 
                ...
                className="flex-1 bg-white py-2 px-4 rounded-lg focus:outline-none"
              />
          ...
          <div className="flex-1 flex items-center justify-center flex-col text-center p-10 bg-gray-50">
            ...
            <h2 className="text-2xl font-light text-gray-600">BidFlow Web</h2>
            <p className="text-gray-400 mt-2">Envie e receba mensagens sem precisar manter seu celular conectado.</p>
          </div>
        )}
      </div>
    </div>
