...
import { ChevronLeft, ChevronRight, Plus, Clock, MapPin, X, Calendar as CalendarIcon, AlignLeft, Trash2, Check, Edit2 } from 'lucide-react';
...
const Calendar = () => {
  ...
  const [events, setEvents] = useState<CalendarEvent[]>([
    { id: 1, title: 'Reunião de Equipe', type: 'meeting', time: '10:00', date: new Date(new Date().getFullYear(), new Date().getMonth(), 5) },
    { id: 2, title: 'Entrega Projeto X', type: 'deadline', time: '17:00', date: new Date(new Date().getFullYear(), new Date().getMonth(), 12) },
    { id: 3, title: 'Call com Cliente', type: 'call', time: '14:30', date: new Date(new Date().getFullYear(), new Date().getMonth(), 15) },
    { id: 4, title: 'Dentista', type: 'personal', time: '18:00', date: new Date(new Date().getFullYear(), new Date().getMonth(), 15) },
    { id: 5, title: 'Lançamento Campanha', type: 'deadline', time: '09:00', date: new Date(new Date().getFullYear(), new Date().getMonth(), 22) },
  ]);
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < startDay; i++) {
      days.push(<div key={`empty-${i}`} className="bg-gray-50/50 min-h-[100px] border border-gray-100 hidden md:block"></div>);
    }
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      days.push(
        <div key={day} className={`min-h-[100px] bg-white border border-gray-100 p-2 relative group hover:bg-gray-50 transition`}>
             ...
             {dayEvents.map(event => (
                <div 
                    ...
                    className={`px-2 py-1 rounded text-xs truncate border-l-2 cursor-pointer hover:opacity-80 transition shadow-sm ${
                        ...
                        event.type === 'deadline' ? 'bg-red-50 border-red-500 text-red-700' :
                        event.type === 'personal' ? 'bg-green-50 border-green-500 text-green-700' :
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">Visualize seus compromissos e agendamentos.</p>
        </div>
                ...
                {events
                    ...
                    >
                        <div className="bg-indigo-50 text-indigo-600 p-2 rounded-lg text-center min-w-[3rem]">
                             <span className="block text-xs font-bold uppercase">{monthNames[event.date.getMonth()].substring(0, 3)}</span>
                             <span className="block text-xl font-bold">{event.date.getDate()}</span>
                        </div>
                        <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                    <h3 className="font-bold text-gray-800">{editingId ? 'Editar Evento' : 'Novo Evento'}</h3>
