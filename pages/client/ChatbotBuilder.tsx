    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[600px] animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#e5ddd5] relative">
          ...
          {history.map((msg, i) =>
            msg.sender === "system" ? (
              ...
              >
                <div className="flex items-center gap-1.5 px-3 py-1 bg-gray-100 border border-gray-200 rounded-full shadow-sm">
        ...
        <div className="p-3 bg-gray-50 border-t border-gray-200 flex gap-2 items-center">
          <input
            className="flex-1 border-0 bg-white shadow-sm rounded-full px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none text-gray-700"
...
const ChatbotBuilder = () => {
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col bg-white border border-gray-200 rounded-xl overflow-hidden shadow-lg relative">
      <TestModal
      ...
      {/* Toolbar */}
      <div className="h-16 bg-gray-50 border-b border-gray-200 flex justify-between items-center px-6 z-10">
        <div className="flex items-center gap-4">
          <h2 className="font-bold text-gray-800">Fluxo de Atendimento</h2>
          <div className="h-6 w-px bg-gray-300 mx-2"></div>
          <div className="flex gap-2">
            <button
              onClick={() => addNode("message")}
              className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-200 text-gray-700 rounded-lg shadow-sm text-sm hover:bg-gray-50 hover:border-indigo-300 transition"
            >
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-gray-700 bg-white hover:bg-gray-50 font-semibold rounded-lg transition border border-gray-200 shadow-sm"
          >
            <Download size={18} /> Baixar Fluxo
          </button>
          <button
            onClick={() => setIsTestModalOpen(true)}
            className="flex items-center gap-2 px-4 py-2 text-indigo-700 bg-indigo-50 hover:bg-indigo-100 font-semibold rounded-lg transition border border-indigo-100"
          >
      ...
      <div
        ref={canvasRef}
        className="flex-1 bg-[#f8fafc] relative overflow-hidden cursor-grab active:cursor-grabbing"
        style={{
      ...
      >
        ...
        {nodes.map((node) => (
          <div
            ...
            className={`absolute w-48 bg-white rounded-xl shadow-lg border-0 ring-1 ring-gray-200 group hover:ring-2 transition-all z-10 ${
              node.type === "start"
          ...
          >
            ...
            {/* Node Body */}
            <div className="p-4">
              <p className="text-sm text-gray-600 line-clamp-3">{node.text}</p>
            </div>

            {/* Ports */}
            <div className="absolute top-1/2 -right-2 w-4 h-4 bg-gray-100 rounded-full border-2 border-gray-300 hover:border-indigo-500 cursor-pointer transition-colors"></div>
            {node.type !== "start" && (
              <div className="absolute top-1/2 -left-2 w-4 h-4 bg-gray-100 rounded-full border-2 border-gray-300 hover:border-indigo-500 cursor-pointer transition-colors"></div>
            )}
          </div>
        ))}
      </div>
